...
void hs_alloc_cb(uv_handle_t* handle, size_t suggested_size, uv_buf_t* buf){
    ...
    size_t nread = loop_data->result_table[slot];
    buf->base = loop_data->read_buffer_table[slot]+nread;      // fetch buffer from buffer table
    buf->len = loop_data->read_buffer_size_table[slot]-nread;  // we ignore suggested_size completely
}
